...
USERS = {
    "admin": {
        "password": hash_password("eventzilla2026"),
        "name": "Administrator",
        "role": "Admin"
    },
    "manager": {
    ...
    "analyst": {
        "password": hash_password("analyst2026"),
        "name": "Data Analyst",
        "role": "Analyst"
    }
}
...
def show_login():
    ...
    <div style="text-align:center;margin-bottom:2rem;">
        ...
        <div style="font-size:2rem;font-weight:700;background:linear-gradient(90deg,#C850C0,#4158D0);
            -webkit-background-clip:text;-webkit-text-fill-color:transparent;">EventZilla</div>
        <div style="color:#8888AA;font-size:0.85rem;letter-spacing:2px;text-transform:uppercase;">
            ML Intelligence Platform</div>
    </div>
    """, unsafe_allow_html=True)
    
    with st.form("login_form"):
        st.markdown('<div style="color:#C850C0;font-weight:600;margin-bottom:0.5rem;">Username</div>', unsafe_allow_html=True)
        username = st.text_input("", placeholder="Enter username", label_visibility="collapsed")
        st.markdown('<div style="color:#C850C0;font-weight:600;margin-bottom:0.5rem;margin-top:1rem;">Password</div>', unsafe_allow_html=True)
    ...
    st.markdown("""
    <div style="text-align:center;margin-top:1rem;color:#8888AA;font-size:0.75rem;">
        Demo credentials:<br>
        admin / eventzilla2026<br>
        manager / manager2026<br>
        analyst / analyst2026
    </div>
    """, unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.divider()
    st.markdown('<div style="color:#8888AA; font-size:0.75rem; text-align:center;">EventZilla AI v2.0<br>MLOps Pipeline Active<br>TechCatalyse © 2026</div>', unsafe_allow_html=True)
    st.divider()
    user_name = st.session_state.get("user_name", "User")
    user_role = st.session_state.get("user_role", "")
    st.markdown(f'''
    <div style="background:rgba(200,80,192,0.1);border:1px solid #C850C0;border-radius:8px;padding:10px;text-align:center;">
        <div style="color:#C850C0;font-size:0.7rem;text-transform:uppercase;letter-spacing:1px;">Logged in as</div>
        <div style="color:white;font-size:0.85rem;font-weight:600;">{user_name}</div>
        <div style="color:#8888AA;font-size:0.7rem;">{user_role}</div>
    </div>
    ''', unsafe_allow_html=True)
    ...
    try:
        _ur.urlopen("http://127.0.0.1:5000", timeout=1)
        st.markdown('<div style="color:#2ECC71;font-size:0.7rem;text-align:center;">● MLflow Active</div>', unsafe_allow_html=True)
    except:
        st.markdown('<div style="color:#E74C3C;font-size:0.7rem;text-align:center;">○ MLflow Offline</div>', unsafe_allow_html=True)
    try:
        _ur.urlopen("http://127.0.0.1:8000", timeout=1)
        st.markdown('<div style="color:#2ECC71;font-size:0.7rem;text-align:center;">● FastAPI Active</div>', unsafe_allow_html=True)
    except:
        st.markdown('<div style="color:#E74C3C;font-size:0.7rem;text-align:center;">○ FastAPI Offline</div>', unsafe_allow_html=True)
...
if page == "Overview":
    ...
    with col_h1:
        ...
        <div style="padding:0.5rem 0;">
            <div style="font-size:1.8rem;font-weight:700;color:#FFFFFF;">EventZilla <span style="background:linear-gradient(90deg,#C850C0,#4158D0);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">Intelligence</span></div>
            <div style="color:#8888AA;font-size:0.85rem;letter-spacing:2px;text-transform:uppercase;">Business Performance & ML Analytics</div>
        </div>
        """, unsafe_allow_html=True)
    with col_h2:
        st.markdown(f"""
        <div class="kpi-card" style="padding:0.8rem;border-color:#4158D0;">
            <div style="color:#4158D0;font-size:0.7rem;text-transform:uppercase;letter-spacing:1px;">Last Updated</div>
            <div style="color:white;font-size:0.95rem;font-weight:600;">{now.strftime("%d %b %Y")}</div>
            <div style="color:#8888AA;font-size:0.75rem;">{now.strftime("%H:%M")}</div>
        </div>
        """, unsafe_allow_html=True)
    with col_h3:
        st.markdown("""
        <div class="kpi-card" style="padding:0.8rem;border-color:#2ECC71;">
            <div style="color:#2ECC71;font-size:0.7rem;text-transform:uppercase;letter-spacing:1px;">System Status</div>
            <div style="color:#2ECC71;font-size:0.95rem;font-weight:600;">● Operational</div>
            <div style="color:#8888AA;font-size:0.75rem;">4 Models Active</div>
        </div>
        """, unsafe_allow_html=True)
    st.markdown("---")
...
# ============================================================
    ...
    with col2:
        ...
        if predict_btn:
            ...
            # Try FastAPI first, fallback to local model
            import urllib.request, json as json_lib
            api_result = None
            try:
                payload = json_lib.dumps({
                    "month": month, "is_weekend": int(is_weekend),
                    "is_summer": int(is_summer), "is_ramadan": int(is_ramadan),
                    "is_holiday": int(is_holiday), "final_price": float(final_price),
                    "price_ratio": float(price_ratio), "event_budget": float(event_budget)
                }).encode()
                ...
                with urllib.request.urlopen(req, timeout=2) as resp:
                    api_result = json_lib.loads(resp.read())
                prob = api_result["cancellation_probability"]
                st.success(f"✅ Prediction via FastAPI — Model: {api_result.get('model_used','')}")
            except:
                prob = predict_cancellation(features)
                st.info("ℹ️ Prediction via local model (API not available)")
            risk_pct = prob * 100
